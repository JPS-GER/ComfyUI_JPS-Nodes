...
class SDXL_Basic_Settings:
    ...
    RETURN_TYPES = ("INT","INT",comfy.samplers.KSampler.SAMPLERS,comfy.samplers.KSampler.SCHEDULERS,"INT","INT","INT","FLOAT","FLOAT","FLOAT","INT","INT","STRING")
    RETURN_NAMES = ("width", "height", "sampler_name", "scheduler", "vae_select", "steps_total", "step_split","cfg_base","cfg_refiner","ascore_refiner","clip_skip_base","clip_skip_refiner","filename")
    FUNCTION = "get_values"
    ...
    def get_values(self,resolution,sampler_name,scheduler,vae_source,steps_total,base_percentage,cfg_base,cfg_refiner,ascore_refiner,clip_skip_base,clip_skip_refiner,filename):
        ...
        return(int(width),int(height),sampler_name,scheduler,int(vae_select),int(steps_total),int(step_split),float(cfg_base),float(cfg_refiner),float(ascore_refiner),int(clip_skip_base),int(clip_skip_refiner),str(filename))
